...
RSpec.describe Grammar::Alternation do
	...
	it 'must require the correct number of arguments' do
	    ...
	    expect { subject.new('abc', 'def') }.to raise_error(ArgumentError)
	end
    end
	...
	it 'must be equal to an equal instance' do
	    ...
	    expect(klass.new('abc')).to eq(klass.new('abc'))
	    expect(klass.new('def')).to eq(klass.new('def'))
	end

	it 'must hash-equal an equal instance' do
	    klass = Grammar::Alternation.with('abc', 'def')
	    expect(klass.new('abc')).to eql(klass.new('abc'))
	end

	it 'must not equal an unequal match' do
	    klass = Grammar::Alternation.with('abc', 'def')
	    expect(klass.new('abc')).not_to eq(klass.new('ghi'))
	end
