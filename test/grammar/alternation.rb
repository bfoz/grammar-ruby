...
RSpec.describe Grammar::Alternation do
	...
	it 'must have a length' do
	    expect(Grammar::Alternation.with('abc', 'abcd').new('abc', location:0).length).to eq(3)
	    expect(Grammar::Alternation.with('abc', 'abcd').new('abcd', location:0).length).to eq(4)
	end
	...
	it 'must equal a matching String' do
	    klass = Grammar::Alternation.with('abc', 'def')
	    expect(klass.new('abc', location:1)).to eq('abc')
	    expect(klass.new('abc', location:1)).not_to eq('def')
	end

	it 'must not equal anything else' do
	    klass = Grammar::Alternation.with('abc', 'def')
	    expect(klass.new('abc', location:1)).not_to eq(Grammar::Concatenation.with('abcdef').new('abcdef', location:1))
	end

	it 'must delegate to the matched alternate' do
	    klass = Grammar::Alternation.with('abc', 'defg')
	    expect(klass.new('abc', location:1).length).to eq(3)
	    expect(klass.new('abc', location:1).location).to eq(1)
	end
    end
end
