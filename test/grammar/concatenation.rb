...
RSpec.describe Grammar::Concatenation do
	...
	it 'must accept the correct number of arguments' do
	    ...
	    expect(subject.new('abc', 'def').elements).to eq(['abc', 'def'])
	end

	it 'must require the correct number of arguments' do
	    expect { subject.new('abc') }.to raise_error(ArgumentError)
	end
    end
	...
	context 'equality' do
		...
		klass = Grammar::Concatenation.with('abc', 'def')
		expect(klass.new('abc', 'def')).to eq(klass.new('abc', 'def'))
	    end
		...
		expect(klass.new('abc', 'def')).not_to eq(klass.new('def', 'abc'))
	    end

	    it 'must not equal anything else' do
		klass = Grammar::Concatenation.with('abc', 'def')
		expect(klass.new('abc', 'def')).not_to eq(Grammar::Alternation.with('abc', 'def').new('abc'))
	    end
	end

	context 'hash equality' do
	    it 'must replace an equal instance' do
		klass = Grammar::Concatenation.with('abc', 'def')
