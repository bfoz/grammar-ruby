...
RSpec.describe Grammar::Concatenation do
	...
	it 'must have a length' do
	    ...
	    expect(klass.new('abc', 'def', location:0).length).to eq(6)
	end

	context 'equality' do
	    it 'must equal an equal String' do
		expect(Grammar::Concatenation.with('abc').new('abc', location:0)).to eq('abc')
		expect(Grammar::Concatenation.with('def').new('def', location:0)).to eq('def')
	    end
	...
	context 'hash equality' do
	    ...
	    it 'must not replace anything else' do
		klass = Grammar::Concatenation.with('abc', 'def')
		expect(klass.new('abc', 'def', location:1)).not_to eql(Grammar::Alternation.with('abc', 'def').new('abc', location:1))
	    end
	end
    end
end
